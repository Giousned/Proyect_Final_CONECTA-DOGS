...
export const AppProvider = ({ children }) => {
  ...
  const handleUpdateUserAndResetDogPhoto = () => {
  ...
  const handleRegister = (e) => {
    ...
    POSTRegister(userInput).then((data) => {
      resetInput({});
  ...
  const handleRegisterDog = (e, dogInput) => {
    ...
    POST_Dog(dogInput).then((data) => {
      actionsToast.handleShownToast(data);

      if (data.code == 200) {
        setTimeout(handleUpdateUserAndResetDogPhoto, 1000);
      }
    });
  };
  ...
  const handleUpdateDog = (e, dogInput, dogId) => {
    ...
    UPDATE_Dog(dogInput, dogId).then((data) => {
      actionsToast.handleShownToast(data);
  ...
  const handleUpdate = (e) => {
    ...
    if (userInput.nurseryDay) {
      newArrayTarrif.push({
        serviceActive: true,
        serviceId: 1,
        price: userInput.priceNurseryDay,
      });
    } else
      newArrayTarrif.push({ serviceActive: false, serviceId: 1, price: 0 });

    if (userInput.walk) {
      newArrayTarrif.push({
        serviceActive: true,
        serviceId: 2,
        price: userInput.priceWalk,
      });
    } else
      newArrayTarrif.push({ serviceActive: false, serviceId: 2, price: 0 });

    if (userInput.nurseryNight) {
      newArrayTarrif.push({
        serviceActive: true,
        serviceId: 3,
        price: userInput.priceNurseryNight,
      });
    } else
      newArrayTarrif.push({ serviceActive: false, serviceId: 3, price: 0 });

    POST_Tariff(newObjTarrifs).then((data) => {
      actionsToast.handleShownToast(data);
    });

    userInput["userPhoto"] = userPhoto;

    UPDATE_User(userInput, storeAuth.userLog.user.id).then((data) => {
      actionsToast.handleShownToast(data);
    });
  };
